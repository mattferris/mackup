...
Snapshots following the naming convention `mackup-<date>-<id>`, or optionally
`mackup-<prefix>-<date>-<id>`. `<date>` is the date the snapshot was created in
the format YYYYMMDD. `<id>` is generated by calculating the MD5 checksum of 512
bytes of data from `/dev/urandom`. `<prefix>` is a user defined value set via
`$ZFS_SNAP_PREFIX`. The script checks for name collisions, and will abort if a
snapshot with the same name exists. Likewise, it honors the ZFS parameter
`snapshot_limit`, and checks this value before attempting a snapshot.

- `$ROTATE` - the number of previous snapshots to keep
- `$ZFS_OPTS` - custom list of options
- `$ZFS_SNAP_PREFIX` - apply a prefix to the snapshot name
